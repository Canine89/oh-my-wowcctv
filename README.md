...
그냥 켜 두면 됩니다. **Dock 아이콘과 창이 없는 메뉴바 앱**이라 실행하면 화면 오른쪽 위 메뉴바에 카메라 아이콘만 생깁니다. 이미 실행 중일 때 다시 열면 CCTV 모니터 창이 뜹니다. 메뉴바 아이콘이 상태를 보여 줍니다.
...
"WoW 창" 모드는 WoW 게임 창을 창 캡처로 직접 잡아 창 내용만 녹화합니다(제목 표시줄 제외). 창 ID 가 바뀌면(재접속 등) 자동으로 다시 잡고, 창 캡처가 프레임을 못 주는 독점 전체 화면에서는 디스플레이 캡처로 자동 대체합니다. 다른 창이 겹쳐도 게임 창만 잡히는 것이 창 캡처의 장점입니다. 창 모드일 때 macOS 제목 표시줄은 잘라냅니다(설정에서 끌 수 있음). 검은 띠 없이 창 내용만 1:1 로 녹화되고, 미리보기도 같은 비율로 보입니다. 창 크기를 바꾸거나 다른 모니터로 옮기면 자동으로 다시 맞춥니다(녹화 중에는 해상도를 바꾸지 않음).

마이크는 사용자가 원래 OBS 에서 쓰던 장치를 그대로 가져오고, CCTV 모니터의 믹서 카드에서 장치를 바꿀 수 있습니다.

**소리 구성**

녹화에는 소리 세 갈래가 따로 들어갑니다. **WoW 소리**(게임), **마이크**(내 목소리), **친구 음성**(디스코드 등 통화 앱 소리). 각각 별도 소스라 CCTV 모니터 믹서에서 따로 보고 볼륨·음소거를 조절할 수 있습니다. 음성 앱은 설정에서 디스코드·줌·슬랙·카카오톡 중 설치된 것으로 고를 수 있고 기본은 디스코드입니다.

**마이크**

기본은 macOS 시스템 기본 입력 장치를 따라갑니다(사운드 설정의 입력). AirPods 를 끼면 AirPods 로, 특정 마이크를 쓰려면 설정 또는 CCTV 모니터의 마이크 카드에서 장치를 고르세요. 마이크 신호가 10초 넘게 없으면 모니터에 경고가 뜹니다. 마이크 필터는 사용자가 원래 OBS 장면에서 마이크에 걸어 둔 체인(노이즈 게이트·소음 억제·컴프레서·리미터 등)을 그대로 복사하고, 이 앱은 그 **맨 앞에 증폭(CCTV 게인, 0~30 dB)** 만 더합니다. 증폭이 앞에 있어야 게이트가 열립니다. 소리가 작으면 증폭을 올리거나 모니터의 **자동 맞춤**을 누르고 5초간 말하면 맞춰 줍니다. macOS 입력 음량도 최대로 올립니다.
