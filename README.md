...
"WoW 창" 모드는 WoW 게임 창을 창 ID 로 직접 캡처(윈도우 캡처)하고, 녹화 해상도를 그 창의 실제 픽셀 크기에 자동으로 맞춥니다. 창 모드일 때 macOS 제목 표시줄은 잘라냅니다(설정에서 끌 수 있음). 검은 띠 없이 창 내용만 1:1 로 녹화되고, 미리보기도 같은 비율로 보입니다. 창 크기를 바꾸거나 다른 모니터로 옮기면 자동으로 다시 맞춥니다(녹화 중에는 해상도를 바꾸지 않음).
